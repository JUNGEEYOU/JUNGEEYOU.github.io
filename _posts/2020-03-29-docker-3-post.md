...
###  OPTIONS
 ...
 | ENTRYPOINT|  <span>&nbsp;</span> 컨테이너가 시작될 때마다 실행할 명령어를 설정.  Dockerfile에서 한 번만 사용 가능. CMD와 다르게 컨테이너 실행 시, 명령어 입력을 하면 반드시 기본으로 실행하며 그 뒤 인자로 명령어가 실행됩니다.  |  <span>&nbsp;</span> ENTRYPOINT 명령, ENTRYPOINT ["실행 파일", "매개 변수1", "매개 변수2"] |
